...
namespace Mutelith {
	...
	public static class AudioDeviceDetector {
		public static AudioDeviceType DetectDefaultDevice(bool verbose = false) {
			using (var enumerator = new AudioDeviceEnumerator()) {
				try {
					var defaultDevice = enumerator.GetDefaultAudioEndpoint(DataFlow.Render, Role.Multimedia);

					if (defaultDevice == null) {
						if (verbose) Logger.Warning("No default audio device found");
						return AudioDeviceType.None;
					}

					using (defaultDevice) {
						string friendlyName = defaultDevice.FriendlyName;
						if (verbose) Logger.Info($"Default device FriendlyName: '{friendlyName}'");

						var deviceInfo = new AudioDeviceInfo(defaultDevice.Device);
						string controllerInfo = deviceInfo.ControllerInfo;
						if (verbose) Logger.Info($"Device Controller: '{controllerInfo}'");

						if (verbose) Logger.Info($"Checking for Sonar: '{SonarConfig.DEVICE_SONAR_CONTROLLER_NAME}'");
						...
						if (verbose) Logger.Info($"Checking for FxSound: '{FxSoundConfig.DEVICE_FXSOUND_CONTROLLER_NAME}' or '{FxSoundConfig.DEVICE_FXSOUND_PREFIX}'");
						...
						if (isSonar) {
							if (verbose) Logger.Info("✓ Using SteelSeries Sonar");
							return AudioDeviceType.SteelSeriesSonar;
						}

						if (isFxSound) {
							if (verbose) Logger.Info("✓ Using FxSound");
							return AudioDeviceType.FxSound;
						}

						if (verbose) Logger.Info("Device not compatible - waiting...");
						return AudioDeviceType.None;
					}
				} catch (Exception ex) {
